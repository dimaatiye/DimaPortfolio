...
GameObject::GameObject(TYPE type)
{
    ...
    if (type == TYPE::NPC) {
        memcpy(this->colour, npc_colours, sizeof(this->colour));
    } else {
...
void GameObject::updateModelMatrix() {
    this->model = glm::translate(mat4(1.0f), this->position);
}
/**
...
void GameObject::setTypeUniform(GLint typeUniformID) {
    glUniform1i(typeUniformID, static_cast<int>(this->type));
}
